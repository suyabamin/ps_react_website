...
export function init() {
  ...
  // Fallback UI when SpeechRecognition API is unavailable
  if (!isSupported) {
    console.warn("SpeechRecognition not supported in this browser.");
    container.innerHTML = `<div class="voice-recognition-card" style="padding:2rem; text-align:center; color:var(--text-primary);">
      <p>Speech Recognition is not supported in this browser.</p>
      <p>Please use a compatible browser like Chrome or Edge.</p>
    </div>`;
    return;
  }
  ...
  // isSupported already verified above; continue with setup
